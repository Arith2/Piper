...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    // auto start_host = std::chrolsno::high_resolution_clock::now();
    // ** FillMissing()
    ...
    const std::streamsize BUFF_SIZE = sizeof(buff);
    const int CHUNK_SIZE = 16; // If 48 integers are read, then each chunk has  16 integers
    
    while (true) {
        // Read a chunk of integers
        file.read(reinterpret_cast<char*>(buff), BUFF_SIZE);
        ...
        // Use memcpy to copy memory chunks
        memcpy(&DDR_table0[DDR_index_0], &buff[0], CHUNK_SIZE * sizeof(int));
        DDR_index_0 += CHUNK_SIZE;
    
        memcpy(&DDR_table1[DDR_index_1], &buff[CHUNK_SIZE], CHUNK_SIZE * sizeof (int));
        DDR_index_1 += CHUNK_SIZE;
    
        memcpy(&DDR_table2[DDR_index_2], &buff[2 * CHUNK_SIZE], CHUNK_SIZE *    sizeof(int));
        DDR_index_2 += CHUNK_SIZE;
    }
...
// ============================================================================
    ...
    auto durationUs_2 = (std::chrono::duration_cast<std::chrono::nanoseconds>(check_point_2-check_point_1).count() / 1000.0);
