...
void LoadDDR(
    ...
    ap_uint<512> input_feature_0 = 0;
    ap_uint<512> input_feature_1 = 0;
    ap_uint<512> input_feature_2 = 0;

    int row_num_total_0 = row_num_0;
    int row_num_total_1 = row_num_0 + row_num_1;
    int row_num_total_2 = row_num_0 + row_num_1 + row_num_2;

    std::cout << "row_num: " << row_num_total_0 << ", " << row_num_total_1 << ", " << row_num_total_2 << ", total: " << row_num_total << std::endl;
    ...
    for (int j = 0; j < 2; j++) {
        for (int i = 0; i < row_num_total; i++) {
            ...
            if (i < row_num_total_0) {
                id = i;
                input_feature_0 = table_DDR0[3*id];
                input_feature_1 = table_DDR0[3*id+1];
                input_feature_2 = table_DDR0[3*id+2];
            }
            else if (i < row_num_total_1) {
                id = i - row_num_total_0;
                input_feature_0 = table_DDR1[3*id];
                input_feature_1 = table_DDR1[3*id+1];
                input_feature_2 = table_DDR1[3*id+2];
            }
            // else if (i < row_num_total_2) {
            //     id = i - row_num_total_1;
            //     input_feature_0 = table_DDR2[3*id];
            //     input_feature_1 = table_DDR2[3*id+1];
            //     input_feature_2 = table_DDR2[3*id+2];
            // }
            else {
                id = i - row_num_total_1;
                input_feature_0 = table_DDR2[3*id];
            ...
            stream_out_0.write(input_feature_0);
            stream_out_1.write(input_feature_1);
            stream_out_2.write(input_feature_2);
        }
    }
}
...
void HexToIntModRange(
    ...
    for (int i = 0; i < 2*row_num; i++) {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            // std::cout << "i: " << i << ", j: " << j << ", " << tmp_input << "%" << max_vocab_size << "=" << tmp_output << " ";
        }
        // std::cout << std::endl;
...
void StoreDDR(
    hls::stream<ap_uint<512> >& stream_in_0,
    hls::stream<ap_uint<512> >& stream_in_1,
    hls::stream<ap_uint<512> >& stream_in_2,
    ap_uint<512>* table_DDR,
    int row_num
) {
    #pragma HLS INLINE off
    ...
    ap_uint<512> input_data_0 = 0;
    ap_uint<512> input_data_1 = 0;
    ap_uint<512> input_data_2 = 0;
    ...
    for (int i = 0; i < row_num; i++) {
        ...
        table_DDR[3*i]   = input_data_0;
        table_DDR[3*i+1] = input_data_1;
        table_DDR[3*i+2] = input_data_2;
    }
