...
void loadData(const std::string& fileName, long startPos, long readBytes, std::vector<ull, aligned_allocator<ull> >& outputArray) {
...
int main(int argc, char** argv)
{
    ...
    ull filelen_per_8 = filelen / 8;

    ull DDR_input0_int = filelen_per_8 / 3;//SINGLE_BUFFER_LIMIT;
    ull DDR_input1_int = filelen_per_8 / 3;//SINGLE_BUFFER_LIMIT;
    ull DDR_input2_int = (filelen % 8 > 0) ? (filelen / 8 + 1 - DDR_input0_int - DDR_input1_int) : (filelen / 8 - DDR_input0_int - DDR_input1_int);

    ull DDR_input0_bytes = DDR_input0_int * 8;
    ull DDR_input1_bytes = DDR_input1_int * 8;
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    std::vector<ull, aligned_allocator<ull >> DDR_table0(DDR_input0_int, 0);
    std::vector<ull, aligned_allocator<ull >> DDR_table1(DDR_input1_int, 0);
    std::vector<ull, aligned_allocator<ull >> DDR_table2(DDR_input2_int, 0);
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    OCL_CHECK(err, cl::Buffer buffer_DDR_table0(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, DDR_input0_int *sizeof(ull), &DDR_table0Ext, &err));
    OCL_CHECK(err, cl::Buffer buffer_DDR_table1(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, DDR_input1_int *sizeof(ull), &DDR_table1Ext, &err));
    OCL_CHECK(err, cl::Buffer buffer_DDR_table2(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, DDR_input2_int *sizeof(ull), &DDR_table2Ext, &err));
